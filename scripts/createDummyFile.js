    ...
    return __generator(this, function (_b) {
      switch (_b.label) {
        case 0:
          ...
          return [
            4 /*yield*/,
            axios.get("https://barttorvik.com/all_ncaa.json"),
          ];
        case 1:
          tournamentsResponse = _b.sent();
          tournaments = tournamentsResponse.data;
          tournamentYears = Object.keys(tournaments).sort(function (a, b) {
            return Number(b) - Number(a);
          });
          lastTournamentYear = tournamentYears.find(function (year) {
            return tournaments[year][team];
          });
          if (lastTournamentYear) {
            clues.push(
              "They last appeared in the NCAA tournament in "
                .concat(lastTournamentYear, " as a ")
                .concat(tournaments[lastTournamentYear][team], " seed.")
            );
          } else {
            clues.push(
          ...
          return [
            4 /*yield*/,
            axios.get(
              "https://barttorvik.com/getgamestats.php?year=2023&tvalue=".concat(
                team
              )
            ),
          ];
        case 2:
          ...
          teamCompletedGames = teamCompletedGamesResponse.data.sort(function (
            a,
            b
          ) {
            return sortableDate(a[0]) - sortableDate(b[0]);
          });
          ...
          if (nonconWins.length) {
            ...
            if (nonconWins.length === 1) {
              ...
              nonconWins[nonconWins.length - 1] = "and ".concat(
                nonconWins[nonconWins.length - 1]
              );
              winsList = nonconWins.join(", ");
            }
            clues.push(
              "In non-conference play, they have beaten ".concat(winsList, ".")
            );
          } else {
        ...
        case 3:
          ...
          highestUsagePlayer = allPlayersResponse.data
            .filter(function (playerInfo) {
              return playerInfo[1] === team && playerInfo[4] > 50;
            })
            .sort(function (a, b) {
              return b[6] - a[6];
            })[0];
          if (highestUsagePlayer) {
            clues.push(
              ""
                .concat(highestUsagePlayer[0], " leads the team with a ")
                .concat(highestUsagePlayer[6], "% usage rate.")
            );
          }
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          ...
          fs.writeFileSync(
            "app/data/dummy.json",
            JSON.stringify({ games: games }, undefined, 2)
          );
          return [2 /*return*/];
      }
    });
